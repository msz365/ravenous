...
let Yelp={
  ...
  getAccessToken(){
    ...
    return(
      fetch(`https://cors-anywhere.herokuapp.com/https://api.yelp.com/oauth2/token?grant_type=client_credentials&client_id=${clientId}&client_secret=${secret}`, {method: POST}).then(
response => {
	if (response.ok) {
